...
#define HOPOZ_QUIT_TIMES 3
#define CTRL_KEY(k) ((k)&0x1f) // define ctrl+character methods

enum editorKey {
  BACKSPACE = 127, // Backspace doesn’t have a human-readable backslash-escape
                   // representation in C (like \n, \r, and so on)
  ARROW_LEFT = 1000, // We will give them a large integer value that is out of
                     // the range of a char, so that they don’t conflict with
                     // any ordinary keypresses.
  ARROW_RIGHT,
  ARROW_UP,
  ARROW_DOWN,
  PAGE_UP,
...
#define HL_HIGHLIGHT_NUMBERS                                                   \
  (1 << 0) // Flag with binary representation 00000001
...
void enableRawMode() {
  ...
  raw.c_cc[VMIN] = 0; // When set to a non-zero value, the read function will
                      // block until at least "vmin" characters are available.
  raw.c_cc[VTIME] =
      1; // When set to a non-zero value, the read function will start a timer
         // after reading "vmin" characters, and if no more characters are
         // received within "vtime" time, the read function will return.
...
int getCursorPosition(int *rows, int *cols) {
  ...
  if (write(STDOUT_FILENO, "\x1b[6n", 4) != 4)
    return -1; // 6n ,request the position of cursor
...
int getWindowsSize(int *rows, int *cols) {
  ...
  if (ioctl(STDOUT_FILENO, TIOCGWINSZ, &ws) == -1 || ws.ws_col == 0) {
    ...
    // ? Why do methods b and c prevent the cursor from jumping out of the
    // screen, but we don’t directly copy the source code of the prevention
    // method?
...
int is_separator(int c) {
  ...
  // strchr() It looks for the first occurrence of a character in a string, and
  // returns a pointer to the matching character in the string.
}
...
void editorOpen(char *filename) {
  ...
  // In practice, size_t is more common than size because it is a type
  // provided by the standard library, has cross-platform compatibility, and
  // is widely used in various standard library functions and types.
...
void editorSave() {
  ...
  int fd = open(E.filename, O_RDWR | O_CREAT,
                0644); // read and write, 0644 is represent for permission
  if (fd != -1) {
...
#define ABUF_INIT                                                              \
  { NULL, 0 } // This acts as a constructor for our abuf type.
  ...
  for (y = 0; y < E.screenrows; y++) {
    ...
    if (filerow >= E.numrows) {
      if (E.numrows == 0 && y == E.screenrows / 3) {
        char welcome[80];
        int welcomelen =
            snprintf(welcome, sizeof(welcome), "HopoZ editor --version %s",
                     HOPOZ_VERSION); // string n print format
      ...
      if (len > E.screencols)
        len = E.screencols;
      char *c = &E.row[filerow].render[E.rowoff];
    ...
    abAppend(ab, "\x1b[K",
             3); // The K command (Erase In Line) erases part of the current
                 // line. Its argument is analogous to the J command’s argument:
    abAppend(ab, "\r\n", 2);
  }
}
...
void editorDrawStatusBar(struct abuf *ab) {
  ...
  int len =
      snprintf(status, sizeof(status), "%.20s - %d lines, modified %d chars",
               E.filename ? E.filename : "[No Name]", E.numrows, E.dirty);
  int rlen = snprintf(rstatus, sizeof(rstatus), "%s | line: %d/%d",
  ...
  // other m command: bold (1), underscore (4), blink (5), and inverted colors
  // (7)
  abAppend(ab, "\r\n", 2);
}
...
void editorRefreshScreen() {
  ...
  snprintf(buf, sizeof(buf), "\x1b[%d;%dH", (E.cy - E.rowoff) + 1,
           (E.rx - E.coloff) + 1);
  // add 1 to E.cy and E.cx to convert from 0-indexed values
...
void editorSetStatusMessage(
    const char *fmt,
    ...) {    // ... makes this function a variadic function
...
void editorProcessKeypress() {
  ...
  case CTRL_KEY('q'):
    if (E.dirty && quit_times > 0) {
      editorSetStatusMessage("WARNING!!! File has unsaved changes."
                             "Press Ctrl-q %d more times to forcely quit.",
                             quit_times);
      quit_times--;
      return;
    }
  ...
  case END_KEY:
    ...
    break;
  case BACKSPACE:
  case CTRL_KEY('h'): // ASCII code 8 is named BS for “backspace”, and ASCII
                      // code 127 is named DEL for “delete”.
                      //  But for whatever reason, in modern computers the
                      //  Backspace key is mapped to 127 and the Delete key
  ...
  case ARROW_RIGHT:
    editorMoveCursor(c);
    break;
  case CTRL_KEY('l'): // Ctrl-L is traditionally used to refresh the screen
...
int main(int argc, char *argv[]) {
  ...
  editorSetStatusMessage(
      "HELP: Ctrl-q to leave | Ctrl-s to save | Ctrl-f or / to find");
